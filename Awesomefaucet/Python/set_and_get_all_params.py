...
if input("Program Device?") in ["Y", "y"]:
    ...
    sensor.write(f'SET:DETECTION_THRESHOLD_MM {2.5 if sensor.ask("GET:SENSOR_TYPE?")=="VL53L4CD" else 2}')  # Try to keeping to 2mm. Increase only if necessary for false trips.
    sensor.write('SET:MAX_DISTANCE_LEAKAGE 0.00333')                                                        # Adjusts the MAX downward tracking leakage. Determines how long a foot can remain wihtout being calibrated away.
    ...
    sensor.write(f'SET:WATER_DEBOUNCE_TIMEOUT {round(2/0.001024)}')                                         # This is the water dwell time after a foot is removed in 1.024ms steps. There will also be a natural dwell from filtering.
